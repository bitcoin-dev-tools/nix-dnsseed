...
let
  ...
  mirrorEnv = {
    HOME = mirror.stateDirectory;
    RAD_HOME = mirror.stateDirectory;
  };
  ...
  mirrorSeed = pkgs.writeShellScript "radicle-mirror-seed" ''
    ...
    rid="$(cat ${lib.escapeShellArg "${mirror.stateDirectory}/${mirror.ridFile}"})"
    ${radSystem} seed "$rid"
  '';
...
in
{
  options.services.radicleMirror.bitcoinMirror = {
    ...
    stateDirectory = lib.mkOption {
      ...
      description = "Absolute state directory for the mirror identity and checkout.";
    };
    ...
    checkoutDirectory = lib.mkOption {
      type = lib.types.str;
      default = "bitcoin";
      description = "Directory name under stateDirectory for the Git checkout.";
    };

    ridFile = lib.mkOption {
      type = lib.types.str;
      default = "bitcoin.rid";
      description = "File under stateDirectory where the generated RID is stored.";
    };
  ...
  config = lib.mkIf (cfg.enable && mirror.enable) {
    ...
    systemd.services.radicle-mirror-init = {
      ...
      serviceConfig = {
        ...
        WorkingDirectory = mirror.stateDirectory;
